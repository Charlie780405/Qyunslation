...
| 问题N，及问题下的（a）（b） | 与当页正文同号（**12pt**） | Bold |
...
正文锁 **12pt** / 章节 **14pt**（+2pt）。疏页只靠行距与段距填版心，**不靠升字号**。

| 项 | 口径 |
| --- | --- |
| 版心 | 左 67pt，右页边 36pt；正文顶 ≈118pt，底到页脚上沿（约 740pt） |
| 常页 | 12pt 估高 ≥ 版心 80%：正文 **12pt**、章节 **14pt**、行距 **1.50**、段距 **12pt** |
| 中疏 / 疏页 | 12pt 估高 < 80%：仍 12/14pt，行距升到 ≤1.90、段距 ≤26pt，目标占 80–88% |
| 短页 | **≤8 行**：仍 12/14pt、行距 1.56、段距 14pt；下空白保留，禁止拉成「两行顶天立地」 |
...
- 检测：`text_boxes` = 排版前原始盒 ∪ 排版后盒；`pad_pt=3.5`。
- 过滤：`aspect > 5` 且 `h_pt < 60` 的细长带丢弃（漏擦正文）。
- 回插：默认只 `logo` / `stamp`；`QYUNSLATION_LETTER_GRAPHICS=1` 才回插 `graphic`。
